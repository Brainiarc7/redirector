...
module Transition
  ...
  class Site < Struct.new(:path, :yaml)
    ...
    def whitehall_slug
      ...
      org = @augmenter.whitehall_organisations.detect { |o| o.title == decoded_title }
    ...
    def save_with_inferred_values!
      File.open(path, 'w') { |file| ordered_output.to_yaml(file) } if whitehall_slug
    end

    def ordered_output
      {}.tap do |ordered_dup|
        ordered_dup['site'] = abbr
        ordered_dup['whitehall_slug'] = whitehall_slug if organisation? && whitehall_slug
